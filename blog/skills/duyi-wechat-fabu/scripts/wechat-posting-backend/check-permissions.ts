...
async function checkApiCredentials(): Promise<void> {
  ...
  const projectEnv = path.join(cwd, '.wechat-article-suite', '.env');
  const userEnv = path.join(os.homedir(), '.wechat-article-suite', '.env');
...
async function main(): Promise<void> {
  console.log('=== wechat-article-suite: Permission & Environment Check ===\n');
